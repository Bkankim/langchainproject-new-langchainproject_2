...
def fetch_competitor_data(
    ...
    """
    경쟁사 제품 데이터 수집 (Fallback: API → Mock)
    ...
    # Fallback Chain: 네이버 쇼핑 API → Mock 데이터
    for i, product_name in enumerate(all_products):
        ...
        try:
            ...
            if api_data:
                ...
                logger.info(f"  ✓ {product_name}: API 데이터 사용")
        except Exception as e:
            logger.warning(f"  ✗ {product_name}: API 호출 실패 ({e})")

        # 2순위: Mock 데이터 (API 실패 시)
        if not product_data:
