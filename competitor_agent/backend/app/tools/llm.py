...
from app.config import SOLAR_API_KEY, SOLAR_MODEL
from app.db.session import get_db
from app.db.crud import get_messages_by_session, append_message

logger = logging.getLogger(__name__)

# Solar Pro 클라이언트 초기화 (OpenAI 호환 API)
client = OpenAI(
...
# 마케팅 에이전트는 키워드 라우팅 방식 사용 (TOOL_DEFINITIONS 미사용)
# 마스터 에이전트 방식으로 전환 시 여기에 마케팅 도구 정의 추가
...
def call_llm(
    ...
    """
    if not client:
        logger.error("Solar Pro 클라이언트가 초기화되지 않았습니다.")
        return {
            "success": False,
            "error": "Solar Pro API 키가 설정되지 않았습니다. .env 파일을 확인하세요.",
...
def call_llm_with_context(
    ...
    if not client:
        ...
        return {
            "success": False,
            "error": "Solar Pro API 키가 설정되지 않았습니다.",
